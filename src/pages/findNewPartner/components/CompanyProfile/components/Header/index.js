...
export default function Header({
  ...
  const excel = () => {
    ...
    const header = [
      {
        ...
        "Company Name": dataCopy.companyName,
    ...
    // Cabeçalho - Export Data
    XLSX.utils.sheet_add_json(exPartners, header, { origin: 0 });
    XLSX.utils.sheet_add_json(exKeyword, header, { origin: 0 });
    XLSX.utils.sheet_add_json(portLading, header, { origin: 0 });
    XLSX.utils.sheet_add_json(exCountries, header, { origin: 0 });
    XLSX.utils.sheet_add_json(ex6digitHs, header, { origin: 0 });

    // Cabeçalho - Import Data
    XLSX.utils.sheet_add_json(imPartners, header, { origin: 0 });
    XLSX.utils.sheet_add_json(imKeyword, header, { origin: 0 });
    XLSX.utils.sheet_add_json(portUnlading, header, { origin: 0 });
    XLSX.utils.sheet_add_json(imCountries, header, { origin: 0 });
    XLSX.utils.sheet_add_json(im6digitHs, header, { origin: 0 });
    XLSX.utils.sheet_add_json(shipments, header, { origin: 0 });
